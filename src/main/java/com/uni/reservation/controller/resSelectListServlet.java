...
/**
 * Servlet implementation class resSelectListServlet
 */
@WebServlet("/memRList.do")
public class resSelectListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public resSelectListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (member != null) {
			...
			if (member.getUserType() == 1 || member.getUserType() == 2) {
				listCount = new ReservationService().getListCount(userNo);
			} else if (member.getUserType() == 3) {
				listCount = new ReservationService().mgetListCount();
			}
			...
			if (member.getUserType() == 1 || member.getUserType() == 2) {
				list = new ReservationService().selectList(userNo, pi);
			} else if (member.getUserType() == 3) {
				list = new ReservationService().mselectList(pi);
			}
			
			request.setAttribute("list", list);
			request.setAttribute("pi", pi);
			request.getRequestDispatcher("views/reservation/MemberReservationSelect.jsp").forward(request, response);
		}
	}
